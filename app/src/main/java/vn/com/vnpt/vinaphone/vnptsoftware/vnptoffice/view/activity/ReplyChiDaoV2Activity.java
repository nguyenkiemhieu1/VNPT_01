...
import vn.com.vnpt.vinaphone.vnptsoftware.vnptoffice.view.event.PersonReplyChiDaoEvent;
...
public class ReplyChiDaoV2Activity extends BaseActivity implements IChiDaoView, ILoginView,
    ...
    private List<String> personEditChiDaos;
    private boolean sendNgay;
    ...
    private void init() {
        ...
        typeReply = getIntent().getStringExtra("typeReply");
        if (typeReply.equals("1")) {
            tvTitle.setText(getResources().getString(R.string.REPLY_CHIDAO));
        } else {
            tvTitle.setText(getResources().getString(R.string.REPLY_ALL_CHIDAO));
        }
        checkSMS.setChecked(false);
        tvTieude.setText(getIntent().getStringExtra("title"));
    ...
    private void getPersonsEdit() {
        if (connectionDetector.isConnectingToInternet()) {
            ...
            chiDaoPresenter.getPersonReply(new ReplyChiDaoRequest(Integer.parseInt(parentId), typeReply.equals("1") ? 1 : 2));
        } else {
            AlertDialogManager.showAlertDialog(this, getString(R.string.NETWORK_TITLE_ERROR), getString(R.string.NO_INTERNET_ERROR), true, AlertDialogManager.ERROR);
        }
    }
    ...
    private void reply() {
        event = "SEND_CHIDAO";
        chiDaoPresenter.send(new SendChiDaoRequest(thongTinId, checkSMS.isChecked() ? "1" : "0", personEditChiDaos.get(0)));
    }
    ...
    private void getPersonReply() {
        event = "GET_PERSON_REPLY";
        chiDaoPresenter.getPersonReply(new ReplyChiDaoRequest(Integer.parseInt(parentId), typeReply.equals("1") ? 1 : 2));
    }

    private void sendNgay() {
        if (connectionDetector.isConnectingToInternet()) {
            sendNgay = true;
            saveInfo();
        } else {
    ...
    private void savePerson() {
        event = "SAVE_PERSON";
        chiDaoPresenter.savePersonChiDao(new SavePersonChiDaoRequest(thongTinId, personEditChiDaos, new ArrayList<String>()));
    }
...
//                    }
            ...
            if (personEditChiDaos != null && personEditChiDaos.size() > 0) {
                for (int i = 0; i < allTree.size(); i++) {
                    for (int j = 0; j < personEditChiDaos.size(); j++) {
                        String idPerson = personEditChiDaos.get(j);
    ...
    @OnClick({R.id.btnSave, R.id.btnSend, R.id.btnSelectFile, R.id.btnSendAll, R.id.btnBack, R.id.btnBackStep1, R.id.btnBackStep2, R.id.btnSavePerson})
    ...
    public void onSuccess(Object object) {
        ...
        if (event != null && event.equals("GET_PERSON_EDIT")) {
            personEditChiDaos = ConvertUtils.fromJSONList(object, String.class);
        ...
        if (event != null && event.equals("GET_PERSON_REPLY")) {
            personEditChiDaos = ConvertUtils.fromJSONList(object, String.class);
            if (personEditChiDaos != null && personEditChiDaos.size() > 0) {
                savePerson();
            }
    ...
    public void onSuccess() {
        ...
        ChiDaoRequest chiDaoRequest = new ChiDaoRequest(thongTinId, tvTieude.getText().toString().trim(),
                tvNoidung.getText().toString().trim(), fileNames, listDeleteFile, parentId, 0);
        chiDaoPresenter.saveChiDao(chiDaoRequest);
    }
    ...
    private void setupSpinnerNhom() {
        myAdapter = new SpinnerCheckboxAdapter(this, 0, lst, "REPLY");
    ...
    public void onBackPressed() {
        ...
        EventBus.getDefault().removeStickyEvent(PersonReplyChiDaoEvent.class);
    ...
    private class ProgressTask extends AsyncTask<Void, Long, Boolean> {
        ...
        protected void onPostExecute(Boolean result) {
            ...
            if (result) {
                if (typeUpdateData == 9) {
                    txtNoData.setVisibility(View.GONE);
                    treeViewOperatingV2 = new TreeViewOperatingV2(root, ReplyChiDaoV2Activity.this, new MyNodeOperatingViewV2Factory());
