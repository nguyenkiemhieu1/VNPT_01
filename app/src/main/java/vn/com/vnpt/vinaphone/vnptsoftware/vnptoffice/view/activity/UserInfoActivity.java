...
public class UserInfoActivity extends BaseActivity implements IUserInfoView, ILoginView, IUserAvatarView, IExceptionErrorView {
    ...
    public void onErrorGetUserInfo(APIError apiError) {
        ...
        if (apiError.getCode() != Constants.RESPONE_UNAUTHEN) {
            AlertDialogManager.showAlertDialog(this, getString(R.string.GET_INFO_TITLE_ERROR), apiError.getMessage() != null && !apiError.getMessage().trim().equals("") ? apiError.getMessage().trim() : "Có lỗi xảy ra!\nVui lòng thử lại sau!", true, AlertDialogManager.ERROR);
        } else {
            if (connectionDetector.isConnectingToInternet()) {
                appPrefs = Application.getApp().getAppPrefs();
